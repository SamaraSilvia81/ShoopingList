...
const Item = ({fruit}) => {
  const {increaseProduct, decrementProduct, removeAllProduct } = useCounterProducts();
  return (
    <View style={styles.item}>
      <Text style={styles.result}>{fruit.title}</Text>
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <TextInput
          ...
          textAlign={'center'}
        />
        <Button mode="contained" onPress={add} disabled={isButtonDisabled}>ADD</Button>
      </View>
      <FlatList
        data={fruits}
        renderItem={({item}) => 
